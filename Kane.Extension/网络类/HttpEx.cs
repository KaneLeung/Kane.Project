...
namespace Kane.Extension
{
    ...
    public class HttpEx
    {
        ...
        /// <param name="token">Authorization标头的Token</param>
        /// <param name="method">压缩算法<see cref="CompressMethod"/></param>
        ...
        public static async Task<string> PostAsync(
            ...
            string token = null,
            CompressMethod method = CompressMethod.None,
            int timeout = 30)
        {
            using HttpClient client = new HttpClient
            {
            ...
            HttpResponseMessage response = await client.PostAsync(url, content);
            response.EnsureSuccessStatusCode();
            return method == CompressMethod.None ? await response.Content.ReadAsStringAsync()
                : new CompressHelper().DeCompressFromByte(await response.Content.ReadAsByteArrayAsync(), method);
        }
        #endregion
